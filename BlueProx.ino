...
const int pinLock = D2; //Digital pins on nRF52832, see  https://github.com/redbear/nRF5x/tree/master/nRF52832
const int pinUnlock = D3; //Digital pins on nRF52832, see  https://github.com/redbear/nRF5x/tree/master/nRF52832
const int pinTrunk = D4; //Digital pins on nRF52832, see  https://github.com/redbear/nRF5x/tree/master/nRF52832
...
void scanCallBack(const Gap::AdvertisementCallbackParams_t *params) {
  uint8_t index;
  if (params->advertisingDataLen > 20) { // need at least 20 bytes for Eddystone-UID, see https://github.com/google/eddystone/tree/master/eddystone-uid
  // Eddystone Parser  
    for (index = 0; index < params->advertisingDataLen; index++){ 
        if (params->advertisingData[index] == 0xAA && params->advertisingData[index+1] == 0xFE && params->advertisingData[index+2] == 0x00) {// Determine if Eddystone prefix exits.
          ...
          if (pCompare(KeyName, UIDName, nLength) == true && pCompare(KeyInstance, UIDInstance, iLength) == true && lockoutRSSI < params->rssi){ //Validate that both UID Namespace and Instance are the same as the known 
            timeoutCount = 0;
              if (lockState == true){ //Car must be locked to be unlocked, otherwise unlock command will be sent everytime this loop runs
                unlock();
                }
            }
                
          else if (pCompare(KeyName, UIDName, nLength) == true && pCompare(KeyInstance2, UIDInstance, iLength) == true && lockOut == 0){ //Validate that both UID Namespace and Instance are the same as the known 
            unlatchTrunk();
                }
...
void loop() {
  ble.waitForEvent();
  if (lockOut>0){ // Start counting down lockOut to enable unlatchTrunk funtion
    lockOut--;
    }

  if (timeoutCount<timeoutLimit){ // Start counting up timoutCount to lock car in next loop
    timeoutCount++;
    }
    
  if (lockState == false && timeoutCount == timeoutLimit){ //If car is unlocked and the timeout counter is reached, then lock the car
    lock();
    }
